...
function ListOfProducts({ product, DeleteProducts, updateProducts, changeView, category }) {
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    if (sortConfig.key) {
      // Handle nested objects for seller and category
      if (sortConfig.key === "seller") {
        const sellerA = a.User.firstName.toLowerCase();
        const sellerB = b.User.firstName.toLowerCase();
        if (sellerA < sellerB) return sortConfig.direction === "asc" ? -1 : 1;
        if (sellerA > sellerB) return sortConfig.direction === "asc" ? 1 : -1;
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            <th onClick={() => requestSort("seller")}>Seller {sortConfig.key === "seller" ? (sortConfig.direction === "asc" ? "↑" : "↓") : "↑↓"}</th>
          ...
          {sortedProducts.map((el) => (
            <tr key={el.id}>
              ...
              <td>
                {show !== el.id ? (
                  <>
                    <button className="btn btn-outline-danger" onClick={() => DeleteProducts(el.id)}>Delete</button>
